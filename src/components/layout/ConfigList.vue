...
import { NList, NListItem, NEmpty, NInput, NButton, NDropdown, NModal, NSpace, useMessage, useDialog } from 'naive-ui'
...
async function handleAddFile() {
  ...
  isAdding.value = true
  try {
    await nexusStore.addFile(nexusStore.selectedCategoryId, newFileName.value.trim())
...
async function handleRenameFile() {
  ...
  try {
    await nexusStore.updateFile(renameFileId.value, { title: renameFileName.value.trim() })
    ...
    <div class="flex-1 overflow-y-auto">
      ...
      <div v-else class="py-2">
        ...
        >
          ...
          <div v-if="item.tags?.length" class="flex items-center mt-1.5 space-x-1.5 flex-wrap gap-y-1">
            <span 
