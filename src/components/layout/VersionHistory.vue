...
async function loadLocalHistory() {
  ...
  try {
    ...
    if (localHistory.value.length > 0) {
        const differentVersion = localHistory.value.find(h => h.content !== props.currentContent);
        if (differentVersion) {
  ...
  try {
    ...
    const versionContents = await Promise.all(
        candidates.map(async (commit) => {
            try {
                const files = await gistRepository.getGistVersion(nexusStore.currentGistId!, commit.version);
                return files && files[props.filename] ? files[props.filename].content : null;
            } catch(e) {
                return null;
            }
        })
...
async function selectRemoteVersion(item: GistHistoryEntry) {
  ...
  try {
    ...
    originalContent.value = file?.content || "(File not found in this version)";
  } catch(e) {
...
async function handleImportHistory() {
    ...
    try {
        const count = await nexusStore.importRemoteHistory(props.fileId, props.filename);
        if (count && count > 0) {
        ...
        <div class="w-1/4 min-w-[300px] flex flex-col border-r pr-4 border-gray-200 dark:border-gray-700 min-h-0">
          ...
          <div class="flex-1 overflow-y-auto min-h-0">
            ...
            <div v-if="activeTab === 'local'">
                <div v-if="localLoading" class="flex justify-center p-4"><NSpin /></div>
                <div v-else-if="localHistory.length === 0" class="flex flex-col items-center justify-center py-8 gap-4">
             ...
             <!-- Remote History List -->
            <div v-else>
                <div v-if="remoteLoading" class="flex justify-center p-4"><NSpin /></div>
            ...
            <div class="mb-4 flex justify-between items-center shrink-0">
                <div class="text-sm text-gray-500 flex items-center gap-4">
                    <div class="flex items-center gap-2">
                        ...
                        <span>左侧: 历史版本 ({{ selectedVersionId ? (activeTab === 'local' ? 'Local Snapshot' : 'Gist Commit') : '未选择' }})</span>
                    </div>
                    <div class="flex items-center gap-2">
                ...
                <div class="flex gap-2">
                     ...
                     <NButton type="warning" @click="handleRestore" :disabled="!selectedVersionId">
                        恢复此版本
                    </NButton>
                </div>
            </div>
